...
{#snippet divider()}
  <div class="w-0.5 bg-orange-600"></div>
{/snippet}

<div class="p-2 rounded-lg bg-[#F88912]">
  <div
    class="flex flex-col border-4 border-orange-600 rounded shadow-lg overflow-hidden"
  >
    {#each rows as _, h}
      ...
      {#if shouldPutDivider(h)}
        <div class="h-0.5 bg-orange-600"></div>
      {/if}
    {/each}
  </div>
</div>
